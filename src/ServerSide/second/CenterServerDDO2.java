...
public class CenterServerDDO2 implements ServerInterface{
...
/*	public CenterServerDDO2() {
		String serverName = "Server_DDO2";
		log(serverName);
	}
	*/
	...
	public static boolean checkManager(String id){
		for(String list: CenterServerDDO2.managers){
			if(list.equals(id)){
				CenterServerDDO2.managerID = id;
				System.out.println("mtl manager id is: "+CenterServerDDO2.managerID);
				return true;
			}
		}
		return false;
	...
	public void openlistener()
	{
		...
		Record rec;
		try{
			ddosocket = new DatagramSocket(6031);
	...
	static class threadrec extends Thread{
		...
		public void run() {
			...
			if(newrecord.m_ID.contains("TR")){
				...
				if(CenterServerDDO2.hashMapDDO.containsKey(firstLetter)){
					new_transferRecord = CenterServerDDO2.hashMapDDO.get(firstLetter);
				...
				synchronized(this){
					CenterServerDDO2.hashMapDDO.put(firstLetter, new_transferRecord);
				}
			}
	...
	class CreateTR extends Thread{
		...
		public void run() {
			...
			String result2 = "ServerDDO2 has receieved the request and processed it ...";
			//send reply
			if(flag_reply){
