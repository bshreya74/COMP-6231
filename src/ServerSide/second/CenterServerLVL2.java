...
public class CenterServerLVL2 implements ServerInterface {
...
/*	
	public CenterServerLVL2() {
		String serverName = "Server_LVL2";
		log(serverName);
	}*/
	...
	public static boolean checkManager(String id){
		for(String list: CenterServerLVL2.managers){
			if(list.equals(id)){
			
				CenterServerLVL2.managerID = id;
				System.out.println("mtl manager id is: "+CenterServerLVL2.managerID);
				return true;
			}
		}
		return false;
	...
	public void openlistener()
	{
		...
		try{
			lvlsocket = new DatagramSocket(6021);
	...
	static class threadrec extends Thread{
		...
		public void run() {
			...
			if(newrecord.m_ID.contains("TR")){
				...
				if(CenterServerLVL2.hashMapLVL.containsKey(firstLetter)){
					new_transferRecord = CenterServerLVL2.hashMapLVL.get(firstLetter);
				...
				synchronized(this){
					CenterServerLVL2.hashMapLVL.put(firstLetter, new_transferRecord);
				}
			}
	...
	class CreateTR extends Thread{
		...
		public void run() {
			...
			String result2 = "ServerLVL2 has receieved the request and processed it ...";
			//send reply
			if(flag_reply){
