...
public class CenterServerMTL2 implements ServerInterface{
...
/*	public CenterServerMTL2() {
		String serverName = "Server_MTL2";
		log(serverName);
	}*/
	...
	public static boolean checkManager(String id){
		for(String list: CenterServerMTL2.managers){
			if(list.equals(id)){
				CenterServerMTL2.managerID = id;
				System.out.println("mtl manager id is: "+CenterServerMTL2.managerID);
				return true;
			}
		}
		return false;
	...
	{	
		...
		Record rec;
		try{
			mtlsocket = new DatagramSocket(6011);
	...
	static class threadrec extends Thread{
		...
		public void run() {
			...
			if(newrecord.m_ID.contains("TR")){
				...
				if(CenterServerMTL2.hashMapMTL.containsKey(firstLetter)){
					new_transferRecord = CenterServerMTL2.hashMapMTL.get(firstLetter);
				...
				synchronized(this){
					CenterServerMTL2.hashMapMTL.put(firstLetter, new_transferRecord);
				}
			}
	...
	class CreateTR extends Thread{
		...
		public void run() {
			...
			String result2 = "ServerMTL2 has receieved the request and processed it ...";
			//send reply
			if(flag_reply){
	...
	class EditRecord extends Thread{
		...
		@Override
		public void run() {
			System.out.println("~~~~~~~~~~~flag reply is:"+flag_reply);
