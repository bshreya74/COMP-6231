...
public class CenterServerMTL1 implements ServerInterface{
	...
	{	
		...
		Record rec;
		try{
			mtlsocket = new DatagramSocket(6010);
	...
	static class threadrec extends Thread{
		...
		public void run() {
			...
			if(newrecord.m_ID.contains("TR")){
				...
				if(CenterServerMTL1.hashMapMTL.containsKey(firstLetter)){
					new_transferRecord = CenterServerMTL1.hashMapMTL.get(firstLetter);
				...
				synchronized(this){
					CenterServerMTL1.hashMapMTL.put(firstLetter, new_transferRecord);
				}
			}
