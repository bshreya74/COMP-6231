...
import ServerSide.first.CenterServerLVL1.threadrec;
...
public class CenterServerDDO1 implements ServerInterface{
...
/*	public CenterServerDDO1() {
	...
	public void openlistener()
	{
		...
		Record rec;
		try{
			ddosocket = new DatagramSocket(6030);
	...
	static class threadrec extends Thread{
		...
		public void run() {
			...
			if(newrecord.m_ID.contains("TR")){
				...
				if(CenterServerDDO1.hashMapDDO.containsKey(firstLetter)){
					new_transferRecord = CenterServerDDO1.hashMapDDO.get(firstLetter);
				...
				synchronized(this){
					CenterServerDDO1.hashMapDDO.put(firstLetter, new_transferRecord);
				}
			}
