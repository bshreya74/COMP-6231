...
public class CenterServerLVL3 implements ServerInterface {
	...
	public static boolean checkManager(String id){
		for(String list: CenterServerLVL3.managers){
			if(list.equals(id)){
			
				CenterServerLVL3.managerID = id;
				System.out.println("mtl manager id is: "+CenterServerLVL3.managerID);
				return true;
			}
		}
		return false;
...
/*	public static void main(String args[]) throws Exception{
		CenterServerLVL3 lvl = new CenterServerLVL3();
	...
	public void openlistener()
	{
		...
		try{
			lvlsocket = new DatagramSocket(6022);
	...
	static class threadrec extends Thread{
		...
		public void run() {
			...
			if(newrecord.m_ID.contains("TR")){
				...
				if(CenterServerLVL3.hashMapLVL.containsKey(firstLetter)){
					new_transferRecord = CenterServerLVL3.hashMapLVL.get(firstLetter);
				...
				synchronized(this){
					CenterServerLVL3.hashMapLVL.put(firstLetter, new_transferRecord);
				}
			}
	...
	class CreateTR extends Thread{
		...
		public void run() {
			...
			String result2 = "ServerLVL3 has receieved the request and processed it ...";
			//send reply
			if(flag_reply){
