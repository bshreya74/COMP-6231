...
public class CenterServerMTL3 implements ServerInterface{
	...
	/*public CenterServerMTL1() {
		String serverName = "Server_MTL1";
		log(serverName);
	}*/
	...
	public static HashMap <Character, LinkedList<Record>> hashMapMTL = new HashMap<Character, LinkedList<Record>>(){
		{
			put('C', new LinkedList<Record>(Arrays.asList(new Record(new TeacherRecord("Zac", "Chen", "5527 rue monsabre", "4389282222", "chinese", "mtl", "TR11111"),"TR11111"))));
			put('G', new LinkedList<Record>(Arrays.asList(new Record(new TeacherRecord("Alex", "Gordin", "2001 rue monsabre", "4389282221", "french", "mtl", "TR11112"),"TR11112"))));
			put('H', new LinkedList<Record>(Arrays.asList(new Record(new StudentRecord("Hao", "Hao", "math", "active", "20170224", "SR01111"), "SR01111"))));
			put('W', new LinkedList<Record>(Arrays.asList(new Record(new StudentRecord("Matt", "Wightman", "history", "active", "20170527", "SR01112"),"SR01112"))));
			put('T', new LinkedList<Record>(Arrays.asList(new Record(new StudentRecord("Jerome", "Targget", "ENGLISH", "active", "20170101", "SR01113"), "SR01113"))));
		}
	};
	...
	public static boolean checkManager(String id){
		for(String list: CenterServerMTL3.managers){
			if(list.equals(id)){
				CenterServerMTL3.managerID = id;
				System.out.println("mtl manager id is: "+CenterServerMTL3.managerID);
				return true;
			}
		}
		return false;
	...
	public String ScreateTRecord(String managerID, String firstName, String lastName, String address, String phone, String specialization,
		...
		if(hashMapMTL.containsKey(firstLetter)){
			records = hashMapMTL.get(firstLetter);
			records.add(r);
		}else{
		...
		synchronized(this){
			hashMapMTL.put(firstLetter, records);
		}
	/*	for(HashMap.Entry <Character, LinkedList<Record>> entry : hashMapMTL.entrySet())
	...
	public String ScreateSRecord(String managerID, String firstName, String lastName, String coursesRegistered, String status, String statusDate){
		...
		/*for(HashMap.Entry entry : hashMapMTL.entrySet())
		{
	...
	public String SgetRecordCounts(String managerID) {
		...
		System.out.println("Manager has requested to know the number of all the records from all the servers. The result is: "+'\n'+count);
		return count;
	}
	...
	public String SeditRecord(String managerID, String recordID, String fieldName, String newValue) {
		...
		if(recordID.contains("TR")){
			for(Map.Entry<Character, LinkedList<Record>> entry : hashMapMTL.entrySet()){
	...
	/*public static void main(String args[]) throws Exception{
		CenterServerMTL3 mtl = new CenterServerMTL3();
		serverName = "Server_MTL1";
		log(serverName);
	...
	{	
		...
		Record rec;
		try{
			mtlsocket = new DatagramSocket(6012);
			...
			while(true){
				...
				DatagramPacket mtlrequest = new DatagramPacket(buffer, buffer.length); 
				mtlsocket.receive(mtlrequest);
				String request = new String(mtlrequest.getData());
				
				int port = mtlrequest.getPort();
				InetAddress address = mtlrequest.getAddress();
				...
				if(request.contains("Hi, can I send a message to you?")){
		            ...
		            mtlsocket.send(sendPacket1);
				}
				
				//get count
				else if(request.contains("GetCount"))
				{
					new thread(mtlsocket, mtlrequest);
				}
				//transfer record
				...
				{	
					...
					DatagramPacket acknowledge = new DatagramPacket(ack, ack.length, mtlrequest.getAddress(), mtlrequest.getPort());
					mtlsocket.send(acknowledge);
					
					//receive object
					DatagramPacket incomingPacket = new DatagramPacket(incomingData, incomingData.length); 
					mtlsocket.receive(incomingPacket);
					...
					try{
						...
						new threadrec(mtlsocket, incomingPacket, rec);
				...
				//transfer record
					...
					new TransferRecord(mtlsocket, mtlrequest, request).start();
				}
			...
			if(mtlsocket != null) 
				mtlsocket.close();
			//if(servermtlsocket != null)
				//servermtlsocket.close();
		}
	}
	...
	static class thread extends Thread{
		...
		public thread(DatagramSocket n_socket, DatagramPacket n_request) {
			...
			mtlcount = getHashSize();
			String MTLcount = Integer.toString(mtlcount);
			result= MTLcount.getBytes();
			this.start();
		}
		...
		public void run() {
			try {
				...
				DatagramPacket mtlreply = new DatagramPacket(result, result.length, request.getAddress(), request.getPort()); 
				socket.send(mtlreply);
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
	...
	static class threadrec extends Thread{
		...
		public void run() {
			try {
				...
				DatagramPacket servermtlreply = new DatagramPacket(rep, rep.length, request.getAddress(), request.getPort()); 
				socket.send(servermtlreply);
			...
			if(newrecord.m_ID.contains("TR")){
				...
				if(CenterServerMTL3.hashMapMTL.containsKey(firstLetter)){
					new_transferRecord = CenterServerMTL3.hashMapMTL.get(firstLetter);
				...
				synchronized(this){
					CenterServerMTL3.hashMapMTL.put(firstLetter, new_transferRecord);
				}
			}
	...
	class CreateTR extends Thread{
		...
		public void run() {
			...
			String result2 = "ServerMTL3 has receieved the request and processed it ...";
			//send reply
			if(flag_reply){
	...
	class GetCount extends Thread{
		...
		public void run() {
			...
			if(flag_reply){
				byte[] response = result1.getBytes();
				System.out.println("########socket port: "+ request.getPort());
				DatagramPacket reply = new DatagramPacket(response, response.length, request.getAddress(), request.getPort());
				try {
					socket.send(reply);
				} catch (IOException e) {
				...
				for(Record record : entry.getValue()){
					if(record.m_ID.equals(recordID)){
						...
						if(transfer.m_ID.contains("TR")){
							...
							new_records = hashMapMTL.get(firstLetter);
							new_records.remove(transfer);
							if(new_records != null){
								synchronized(this){
								hashMapMTL.put(firstLetter, new_records);
								}
							}else{
								hashMapMTL.remove(firstLetter);
							}
						}
